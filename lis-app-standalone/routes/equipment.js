...
router.get('/:id/qc/levey-jennings', requireAuth, (req, res) => {
  try {
    const analyteCode = req.query.analyteCode || req.query.analyte;
    const { controlId, month, startDate, endDate, sort } = req.query;

    if (!analyteCode) {
      return res.status(400).json({ success: false, error: 'Analyte code is required (e.g. ?analyteCode=fbs or ?analyte=fbs).' });
    }
...
router.get('/:id/qc/print', requireAuth, (req, res) => {
  try {
    const { controlId, month, startDate, endDate, sort } = req.query;
    const targetAnalyte = req.query.analyteCode || req.query.analyte || 'fbs';
    const rawEq = global.db.getEquipmentById(req.params.id);
