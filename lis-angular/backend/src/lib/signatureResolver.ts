 ...
 */
export function sanitizeTestSignatures(populatedTest: any): any {
  if (!populatedTest) return populatedTest;

  if (typeof populatedTest.results === 'string') {
    try {
      populatedTest.results = JSON.parse(populatedTest.results);
    } catch (_) {
  ...
  if (typeof populatedTest.results.signatures === 'string') {
    try {
      populatedTest.results.signatures = JSON.parse(populatedTest.results.signatures);
    } catch (_) {
  ...
  for (const [key, sig] of Object.entries(sigs)) {
    ...
    const sigObj = sig as any;
    let filename = sigObj.filename;
    let exists = false;
    if (filename) {
      try {
    ...
    if (!exists) {
      const user = allUsers.find(u =>
        ...
        (u.name && sigObj.name && String(u.name).trim().toLowerCase() === String(sigObj.name).trim().toLowerCase()) ||
      ...
      sigObj.filename = resolvedFilename;
    }
  }
  ...
  const requestedName = (populatedTest.results && populatedTest.results.requestedByName) ||
                        (populatedTest.requested_by_name) ||
                        (populatedTest.requestedBy && populatedTest.requestedBy.name) || '';

  if (requestedName) {
    const hasRequestedSig = Object.values(sigs).some((s: any) =>
