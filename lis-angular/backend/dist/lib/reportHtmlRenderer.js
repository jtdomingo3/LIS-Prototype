...
function calcAge(dob, ageManual) {
    if (dob) {
        const born = new Date(dob);
        if (!isNaN(born.getTime()))
            return String(Math.max(0, new Date().getFullYear() - born.getFullYear()));
    }
...
function getInlineLogo() {
    if (typeof _cachedInlineLogo !== 'undefined')
...
function getSignatureDataUri(filename) {
    try {
        const candidates = [
...
// ── signatures ─────────────────────────────────────────────────────────
function findSig(results, displayName) {
    if (!displayName || !results?.signatures)
        return null;
    const sigs = results.signatures;
    for (const k of Object.keys(sigs)) {
        const s = sigs[k];
        if (s?.name && String(s.name).trim() === String(displayName).trim())
            return s;
    }
    return null;
}
...
// ── HEMATOLOGY / CBC ───────────────────────────────────────────────────
function renderHematology(r, sex) {
    const _sex = (sex || '').toLowerCase();
    function sexRow(key, mRange, fRange, mMin, mMax, fMin, fMax) {
        const val = r[key] ?? '';
    ...
    function simpleRow(label, key, ref, min, max) {
        const val = r[key] ?? '';
...
function renderBloodChemistry(r, title) {
...
function renderFecalOccultBlood(r) {
    return `
...
// ── PREGNANCY TEST ─────────────────────────────────────────────────────
function renderPregnancyTest(r) {
    return `
...
function renderGenericBody(r, title) {
...
//  DISPATCH
// ═══════════════════════════════════════════════════════════════════════
function renderBody(template, results, sex, testType) {
    const r = results || {};
    switch (template) {
...
//  PUBLIC API
// ═══════════════════════════════════════════════════════════════════════
function renderReportHtml(test, patient, baseUrl, options) {
...
//# sourceMappingURL=reportHtmlRenderer.js.map