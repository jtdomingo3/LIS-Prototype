...
function normalizeTestType(raw) {
    ...
    if (/blood\s*chemistry|blood-chemistry|\bbc\b|bun|crea|sgpt|sgot|lipid|hba1c|albumin|blood[-_\s]?sugar/i.test(lower))
        return 'Blood Chemistry';
    if (/ultrasound|ultrasonography|sonography|usg|abdomen sonography|abdominal ultrasound/i.test(lower))
        return 'Ultrasound';
 ...
 */
router.get('/', (0, auth_1.requirePermission)('dashboard'), (req, res) => {
    try {
        ...
        // Patient counts
        const totalPatients = Patient_1.PatientModel.count();
      ...
      `).get(selectedDateStr);
        ...
        try {
            const allPatients = db.prepare('SELECT payment_history FROM patients').all();
        ...
        try {
            const allTests = db.prepare('SELECT test_type, test_date, created_at, requested_tests FROM tests').all();
            for (const t of allTests) {
                const requestedTests = JSON.parse(t.requested_tests || '[]');
                const candidates = requestedTests.length === 0
                    ? [t.test_type || '']
                    : requestedTests.map((r) => (r?.label || r?.key) || r);
      ...
      `).all();
        ...
        const typeBreakdown = Test_1.TestModel.countByType();
        return res.json({
            stats: {
                totalPatients,
                totalTests,
