...
const reportHtmlRenderer_1 = require("../lib/reportHtmlRenderer");
...
router.get('/', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        ...
        const countRow = db.prepare(`SELECT COUNT(*) as count FROM tests t LEFT JOIN patients p ON t.patient_id = p.id ${where}`).get(...params);
    ...
    `).all(...params, lim, offset);
        ...
        const typeRows = db.prepare("SELECT DISTINCT test_type FROM tests WHERE status IN ('Completed', 'Released') AND test_type IS NOT NULL ORDER BY test_type").all();
        const availableTestTypes = typeRows.map((r) => r.test_type);
        return res.json({
            tests,
 ...
 */
router.get('/nav', (0, auth_1.requirePermission)('reports'), (_req, res) => {
    try {
        const db = (0, connection_1.getDb)();
        const rows = db.prepare(`
    ...
    `).all();
        ...
        items.forEach((i) => { if (i.patientName && i.patientName !== 'Unknown')
            patientSet.add(i.patientName); });
        const patients = Array.from(patientSet).sort();
        // unique test types
        const typeSet = new Set();
        items.forEach((i) => { if (i.testType)
            typeSet.add(i.testType); });
...
router.post('/print-multiple', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        ...
        const pages = [];
        for (const id of ids) {
            const test = Test_1.TestModel.findById(id);
            if (!test)
                continue;
 ...
 */
router.get('/worksheet/types', (0, auth_1.requirePermission)('reports'), (_req, res) => {
    try {
        const db = (0, connection_1.getDb)();
        const rows = db.prepare("SELECT DISTINCT test_type FROM tests WHERE test_type IS NOT NULL ORDER BY test_type").all();
        const types = rows.map((r) => r.test_type);
        // Get unique companies for patient export
        const compRows = db.prepare("SELECT DISTINCT company FROM patients WHERE company IS NOT NULL AND company != '' ORDER BY company").all();
        const companies = compRows.map((r) => r.company);
 ...
 */
router.get('/:id/html', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        ...
        const html = (0, reportHtmlRenderer_1.renderReportHtml)(test, patient, baseUrl, { print: printMode });
        res.setHeader('Content-Type', 'text/html; charset=UTF-8');
 ...
 */
router.get('/:id', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        const test = Test_1.TestModel.findById(req.params.id);
        if (!test)
 ...
 */
router.post('/worksheet/preview', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
 ...
 */
router.post('/worksheet/download', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
 ...
 */
router.post('/patient-export/preview', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        ...
        const rows = db.prepare(`${query} ${where} ORDER BY p.created_at DESC LIMIT 200`).all(...params);
        ...
        const previewRows = rows.map((r) => {
            ...
            if (dateStart && dateEnd) {
                const tr = db.prepare(`SELECT test_type FROM tests WHERE patient_id = ? AND DATE(test_date) BETWEEN DATE(?) AND DATE(?) ORDER BY test_date LIMIT 1`).get(r.id, dateStart, dateEnd);
                if (tr && tr.test_type)
 ...
 */
router.post('/patient-export/download', (0, auth_1.requirePermission)('reports'), (req, res) => {
    try {
        const { dateFrom, dateTo, company, philhealth } = req.body;
        const db = (0, connection_1.getDb)();
        let where = 'WHERE 1=1';
        ...
        const rows = db.prepare(`${query} ${where} ORDER BY p.created_at DESC`).all(...params);
        ...
        for (const r of rows) {
            ...
            let testType = '';
            if (dateFrom && dateTo) {
                const tr = db.prepare(`SELECT test_type FROM tests WHERE patient_id = ? AND DATE(test_date) BETWEEN DATE(?) AND DATE(?) ORDER BY test_date LIMIT 1`).get(r.id, dateFrom, dateTo);
                if (tr && tr.test_type)
                    testType = tr.test_type;
            }
            const vals = [
