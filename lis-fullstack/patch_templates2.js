...
// We will only replace \`fields: [],\` that are inside the \`getStaticResultTemplates\` function.
// Since \`fields: [],\` only occurs in this file inside that function, a global replace is safe.
content = content.replace(/fields:\s*\[\],\s*createdAt:\s*null,\s*isStatic:\s*true/g, 'fields: (defaultFieldsMapping[f] || []),\n          createdAt: null,\n          isStatic: true');

fs.writeFileSync(filePath, content, 'utf8');
console.log('Successfully patched templates.js exactly.');
