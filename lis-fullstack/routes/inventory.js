...
router.post('/', requireInventoryAccess, (req, res) => {
  try {
    const { 
      name, sku, category, unit, packageSize, minThreshold, criticalThreshold, maxThreshold, 
      supplier, supplierPartNumber, manufacturer, cost, area, location, 
      storageTemp, requiresRefrigeration, hazardClass, msdsUrl, 
      openVialStabilityDays, barcode, notes 
    } = req.body;
...
const handleUpdateItem = (req, res) => {
  try {
    ...
    const { 
      ...
      supplier, supplierPartNumber, manufacturer, cost, area, location, 
      storageTemp, requiresRefrigeration, hazardClass, msdsUrl, 
      openVialStabilityDays, barcode, notes, isActive 
    } = req.body;
    ...
    const updatedItem = new Inventory({
      ...
      category: category !== undefined ? category : raw.category,
...
const handleDeleteItem = (req, res) => {
  try {
    ...
    const success = global.db.deleteInventory(req.params.id);
    if (!success) {
